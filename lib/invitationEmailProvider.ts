...
export const sendInvitationEmail = async ({
    ...
    await transport.sendMail({
        ...
        text: text({url, site}),
        html: html({url, site, email, data}),
    })
}

// Email HTML body
const html = ({ url, site, email, data }) => {
  ...
  <table width="100%" border="0" cellspacing="20" cellpadding="0" style="background: ${mainBackgroundColor}; max-width: 600px; margin: auto; border-radius: 10px;">
    <tr>
      <td align="center" style="padding: 10px 0px 0px 0px; font-size: 18px; font-family: Helvetica, Arial, sans-serif; color: ${textColor};">
        <strong>${escapedEmail}</strong>, you have been invited by <strong>${data.inviterName} (${data.inviterEmail})</strong> to <strong>${data.biotopeName}</strong>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td align="center" style="padding: 0px 0px 10px 0px; font-size: 16px; line-height: 22px; font-family: Helvetica, Arial, sans-serif; color: ${textColor};">
        Joining ${data.biotopeName} allows you to participate in the community.
      </td>
    </tr>
  </table>
</body>
